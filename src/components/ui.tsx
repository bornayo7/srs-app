...
const variants = {
  ...
  danger:
    'bg-rose-900/60 hover:bg-rose-800 hover:text-white text-rose-100 border border-rose-800 disabled:opacity-50',
...
export function Field({
  ...
  const group = !isValidElement(child) || child.type === 'div' || child.type === 'span';
  if (group)
    return (
      <fieldset className={`min-w-0 ${className}`}>
        <legend className="mb-1 text-sm text-slate-400">{label}</legend>
        {children}
...
export function Modal({
  ...
  return (
    <dialog
      ...
      className={`app-dialog ${wide ? 'max-w-4xl' : 'max-w-2xl'}`}
      onMouseDown={(e) => {
    ...
    >
      <div className="w-full rounded-xl border border-slate-800 bg-slate-900 shadow-2xl">
        <header className="flex items-center justify-between border-b border-slate-800 px-4 py-3">
          <h2 id={titleId} className="text-base font-semibold text-slate-200">
            {title}
          </h2>
          <Button variant="ghost" onClick={onClose} aria-label="Close dialog" title="Close (Esc)">
            ✕
          </Button>
        </header>
...
export function Status({ error, message }: { error?: string; message?: string }) {
  return (
    <>
      {error && (
...
export function ButtonLink({
  ...
  return (
    <Link
      to={to}
      className={`inline-flex min-h-10 items-center justify-center rounded-lg px-3 py-2 text-sm font-medium ${variants[variant]} ${className}`}
    >
      {children}
    </Link>
  );
